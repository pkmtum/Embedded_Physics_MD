...
def write_command(file_name, arguments):
    with open(file_name, 'w') as f:
        f.write('python '+' '.join(arguments))
        f.write('\n')
    f.close()
...
def plt_kldiv(x,y, path, filename, labelname=None):

    f, ax = plt.subplots(1)
    yt = y - min(y) + 1.e-6
    if labelname is None:
        ax.semilogy(x, yt, label=r'$D_{\text{KL}}( p(x) || \bar p_{\theta}(x) )$')
    else:
        ax.semilogy(x, yt, label=labelname)
...
def plt_kldiv_combined(x,y, path, filename, labelname):
    ls_list = ['-', '--']

    f, ax = plt.subplots(1)
    for i, y_kl_part in enumerate(y):
        y_kl_part = y_kl_part - min(y_kl_part) + 1.e-6
        ax.semilogy(x, y_kl_part, label=labelname[i], lw=3, ls=ls_list[i])
    ax.set_ylabel('KL-Divergence')
    ax.set_xlabel('Iteration')
    ax.grid(ls='dashed')
    ax.legend()
